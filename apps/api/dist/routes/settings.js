...
router.put('/:guildId', requireGuildAccess, async (req, res) => {
    try {
        ...
        const settings = await GuildSettingsModel.findOneAndUpdate({ guildId }, filteredUpdates, { upsert: true, new: true });
        res.json({
            success: true,
            data: settings,
...
router.post('/:guildId/reset', requireGuildAccess, async (req, res) => {
    try {
        const { guildId } = req.params;
        await GuildSettingsModel.findOneAndUpdate({ guildId }, {
            prefix: '!',
            djRoleId: null,
            defaultVolume: 100,
            maxQueueLength: 1000,
