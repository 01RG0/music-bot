...
io.use(async (socket: AuthenticatedSocket, next) => {
  ...
  io.on('connection', (socket: AuthenticatedSocket) => {
    ...
    socket.on('web:play', async (data: { guildId: string; query: string }) => {
      ...
      try {
        ...
        const tracks = await global.queueManager.searchAndAdd(
          ...
          data.query,
          socket.discordId!,
          (q) => global.playerManager.search(q)
        );
    ...
    socket.on('web:control', async (data: { guildId: string; action: string; params?: any }) => {
      if (!socket.guilds?.includes(data.guildId)) {
        socket.emit('error', { message: 'Access denied' });
        return;
      }
    ...
    socket.on('web:queue', async (data: { guildId: string; action: string; params?: any }) => {
      if (!socket.guilds?.includes(data.guildId)) {
        socket.emit('error', { message: 'Access denied' });
        return;
      }
    ...
    socket.on('web:loop', async (data: { guildId: string; mode: 'off' | 'track' | 'queue' }) => {
      if (!socket.guilds?.includes(data.guildId)) {
        socket.emit('error', { message: 'Access denied' });
        return;
      }
    ...
    socket.on('web:autoplay', async (data: { guildId: string; enabled: boolean }) => {
      if (!socket.guilds?.includes(data.guildId)) {
        socket.emit('error', { message: 'Access denied' });
        return;
      }
...
function setupEventForwarding(io: Server) {
  // Player events
