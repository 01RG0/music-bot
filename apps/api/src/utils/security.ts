...
const guildCooldowns = new Map<string, Map<string, number>>();
...
export class SecurityManager {
  ...
  static async checkCommandPermission(
    ...
    try {
      ...
      const commandPermissions = settings.permissions[command as keyof typeof settings.permissions];
  ...
  // Check for spam
  static checkSpam(userId: string): { isSpam: boolean; resetTime?: number } {
    const now = Date.now();
  ...
  // Validate input data
  static validateTrackQuery(query: string): { valid: boolean; reason?: string } {
...
export async function requireCommandPermission(command: string) {
...
export function requireCooldown(command: string, cooldownMs: number = 3000) {
