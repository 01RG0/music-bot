...
import { QueueModel, GuildSettingsModel } from '@utils/schemas';
export class QueueManager extends EventEmitter {
    ...
    async saveQueue(guildId, updates) {
        try {
            await QueueModel.updateOne({ guildId }, { ...updates, updatedAt: new Date() }, { upsert: true });
            // Update cache
    ...
    // Add tracks to queue
    async addTracks(guildId, tracks, position) {
    ...
    // Get next tracks
    async getNextTracks(guildId, limit = 10) {
        const queue = await this.loadQueue(guildId);
        if (!queue)
            return [];
    ...
    async setPlayingStatus(guildId, isPlaying) {
        await this.saveQueue(guildId, { isPlaying });
    }
    // Search and add tracks
    async searchAndAdd(guildId, query, requester, searchFunction) {
        try {
